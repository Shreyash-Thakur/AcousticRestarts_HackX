...
async function main() {
  ...
  // ── 5. Groth16Verifier (auto-generated by circom build) ────────────────────
  console.log("\n[5/6] Deploying Groth16Verifier...");
  ...
  // ── 6. RiskScoreRegistry (ZK verifier wrapper) ────────────────────────────
  console.log("\n[6/6] Deploying RiskScoreRegistry...");
  const RiskScoreRegistry = await ethers.getContractFactory("RiskScoreRegistry");
  const riskRegistry = await RiskScoreRegistry.deploy(
    groth16VerifierAddr, // real Groth16Verifier
    deployer.address     // admin
  );
  await riskRegistry.waitForDeployment();
  const riskRegistryAddr = await riskRegistry.getAddress();
  console.log("    RiskScoreRegistry :", riskRegistryAddr);
...
══════════════════════════ DEPLOYMENT SUMMARY ══════════════════════════
  ...
  RiskScoreRegistry    : ${riskRegistryAddr}
═══════════════════════════════════════════════════════════════════════

Add to your .env:
  CONTRACT_ADDRESS=${invoTokenAddr}
  INVOICE_CONTRACT_ADDRESS=${invoTokenAddr}
  FUNDING_POOL_ADDRESS=${fundingPoolAddr}
  RISK_REGISTRY_ADDRESS=${riskRegistryAddr}
  GST_VERIFIER_ADDRESS=${gstVerifierAddr}
...
Next steps:
  ...
  2. Run \`npm run chainlink:configure\` to set the GST verification JS source
`);
}
